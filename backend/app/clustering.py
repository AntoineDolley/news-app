...
import json
from .utils.openai import generate_summary_and_title  # Votre fonction de résumé
...
def workflow_query_cluster_and_summarize(articles, k_range: range = range(2, 21)):
    ...
    # Générer résumé et titre pour chaque cluster
    cluster_summaries = {}
    for cluster_id in sorted(df['cluster'].unique()):
        ...
        print(f"Titre : {summary_and_title.get('title')}")
        print(f"Résumé : {summary_and_title.get('summary')}\n")
